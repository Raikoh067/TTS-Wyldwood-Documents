...
function doPost(e) {
  try {
    ...
    // Generate game ID server-side (ignore any gameId sent from TTS)
    const gameId = generateNextGameId(ss);
    data.gameId = gameId;
...
// GAME ID GENERATION
// ============================================
function generateNextGameId(ss) {
  const sheet = ss.getSheetByName(CONFIG.MATCH_RESULTS_SHEET);
  if (!sheet) {
    Logger.log('Warning: MatchResults sheet not found, starting at game ID 1');
    return '00001';
  }
  
  const data = sheet.getDataRange().getValues();
  if (data.length <= 1) {
    // Only header row or empty - this is the first game
    return '00001';
  }
  
  // Find the gameId column (should be column 2, index 1)
  const headers = data[0];
  const gameIdCol = headers.indexOf('gameId');
  
  if (gameIdCol === -1) {
    Logger.log('Warning: gameId column not found, starting at game ID 1');
    return '00001';
  }
  
  // Find the highest game ID
  let maxGameId = 0;
  for (let i = 1; i < data.length; i++) {
    const cellValue = data[i][gameIdCol];
    if (cellValue) {
      // Parse the game ID (handle both numeric and string formats)
      const numericId = parseInt(String(cellValue).replace(/\D/g, ''), 10);
      if (!isNaN(numericId) && numericId > maxGameId) {
        maxGameId = numericId;
      }
    }
  }
  
  // Increment and format as 5-digit string
...
function testScript() {
  try {
    ...
    // Generate game ID server-side (like production)
    const gameId = generateNextGameId(ss);
    Logger.log('Generated game ID: ' + gameId);
    ...
    if (cooldownResult.allowed) {
      ...
      Logger.log('Test completed successfully! Game ID: ' + gameId);
    } else {
...
The TTS script sends data in this format (gameId is generated server-side):
...
**Note:** The `gameId` is generated by Google Sheets server-side by finding the highest existing game ID and incrementing by 1. This ensures consistent sequential IDs even if TTS data is not persisted.
